...
<head>
    ...
    <title>Wallet Information - Segro</title>
...
<body>
    ...
    <div class="container">
        ...
        @if ($errors->any())
            <div class="alert alert-error" style="margin-bottom: 20px;">
                ...
                <ul style="margin: 0; padding-left: 20px; color: #ef4444; font-size: 13px; list-style-type: disc;">
        ...
        <div class="card">
            <h2 class="card-title">Link Payout Wallet</h2>
            <p style="font-size: 13px; color: var(--text-secondary); margin-bottom: 20px;">Bound wallet information will be used for withdrawing funds securely.</p>

            @if($wallet)
                <!-- Bound Wallet Display -->
                <div class="payout-card">
                    <div class="payout-card-header">
                        <div class="payout-card-logo">
                            <div class="payout-card-logo-icon">₮</div>
                            <div class="payout-card-logo-text">USDT Payout Wallet</div>
                        </div>
                        <div style="display: flex; gap: 8px;">
                            <span class="payout-card-network-badge">{{ $wallet->type }}</span>
                            @if($wallet->blockchain)
                                <span class="payout-card-network-badge" style="background: rgba(16, 185, 129, 0.15); color: var(--success-color); border-color: rgba(16, 185, 129, 0.3);">{{ $wallet->blockchain }}</span>
                            @endif
                        </div>
                    </div>
                    <div class="payout-card-address">{{ $wallet->vallet_address }}</div>
                ...
                <p style="font-size: 12px; color: var(--text-muted); text-align: center; margin-top: 15px;">To modify your linked withdrawal address, please contact our support desk.</p>
            @else
                <!-- Form to bind wallet -->
                <form method="POST" action="{{ route('wallet-information.save') }}">
                    ...
                    <div class="form-group">
                        <label for="vallet-address">Wallet address</label>
                        <input type="text" id="vallet-address" name="vallet-address" placeholder="Enter USDT address" required>
                    </div>

                    <div class="form-group">
                        <label>Select network/type</label>
                        <div class="preset-chips">
                            <label class="preset-chip active" style="display: block; cursor: pointer;">
                                <input type="radio" name="wallet-type" value="TRC20" checked style="display:none;"> TRC20
                            </label>
                            <label class="preset-chip" style="display: block; cursor: pointer;">
                                <input type="radio" name="wallet-type" value="ERC20" style="display:none;"> ERC20
                            </label>
                            <label class="preset-chip" style="display: block; cursor: pointer;">
                                <input type="radio" name="wallet-type" value="ETH" style="display:none;"> ETH
                            </label>
                            <label class="preset-chip" style="display: block; cursor: pointer;">
                                <input type="radio" name="wallet-type" value="BTC" style="display:none;"> BTC
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="preset-chips">
                            <label class="preset-chip active" style="display: block; cursor: pointer;">
                                <input type="radio" name="blockchain" value="TRON" checked style="display:none;"> TRON (TRC20)
                            </label>
                            <label class="preset-chip" style="display: block; cursor: pointer;">
                                <input type="radio" name="blockchain" value="Ethereum" style="display:none;"> Ethereum (ERC20)
                            </label>
                            <label class="preset-chip" style="display: block; cursor: pointer;">
                                <input type="radio" name="blockchain" value="Bitcoin" style="display:none;"> Bitcoin
                            </label>
                            <label class="preset-chip" style="display: block; cursor: pointer;">
                                <input type="radio" name="blockchain" value="BSC" style="display:none;"> BSC (BEP20)
                            </label>
                        </div>
                    </div>

                    <button type="submit" class="btn btn-primary" style="margin-top: 10px;">Link Wallet</button>
                </form>
            @endif
        </div>
    </div>
    ...
    <script src="{{ asset('js/pages/wallet-info.js') }}"></script>
