...
<head>
    ...
    <title>Membership Levels - Segro</title>
...
<body>
    ...
    <div class="container">
        ...
        @foreach($memberships as $index => $item)
            @php
                $isUnlocked = $item->id <= $currentUserLevelId;
            @endphp

            <div class="tier-card {{ $isUnlocked ? 'unlocked' : '' }}">
                <div class="tier-header">
                    <!-- Brand names representing elite levels -->
                    <h3 style="font-size: 17px; font-weight: 700; color: var(--text-primary);">
                        ...
                        @elseif($index == 1)
                            Alibaba Concierge
                        @elseif($index == 2)
                            AliExpress Concierge
                        @else
                            {{ $item->level_name }}
                        @endif
                    </h3>

                    <span class="tier-badge {{ $isUnlocked ? 'unlocked' : 'locked' }}">
                        {{ $isUnlocked ? 'Unlocked' : 'Locked' }}
                    </span>
                </div>

                <div class="tier-range">
                    Daily Limit: {{ $item->order_limit }} orders
                </div>
                ...
                <div class="tier-actions">
                    @if($isUnlocked)
                        <a href="/data-optimization" class="btn btn-primary">Start Reviewing</a>
                    @else
                        <a href="/recharge" class="btn btn-secondary">Add Funds</a>
                    @endif
                </div>
            </div>
        @endforeach

        <!-- Static original image at bottom for backward compatibility -->
        <div class="card text-center" style="margin-top: 20px;">
            <h4 class="card-title">Membership Roadmap</h4>
            <img src="{{ asset('images/activity.jpg') }}" alt="Plans Overview" style="width: 100%; border-radius: 8px; margin-top: 10px;">
        </div>
    </div>
